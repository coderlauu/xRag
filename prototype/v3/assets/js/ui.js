(function () {
  ...
  function navMarkup(active) {
    var items = [
      ...
      { key: "search", href: "search.html", label: "搜索增强" },
      { key: "detail", href: "detail.html", label: "文档时间线" },
  ...
  function docCard(doc, extra) {
    var sourceMeta = doc.source_type === "link" ? (doc.source_url || "链接来源") : (doc.file_name || "文件");
    return (
      '<article class="list-card">' +
      '<div class="list-card__head">' +
      '<div>' +
      '<h4 class="list-title"><a href="detail.html?doc=' + encodeURIComponent(doc.id) + '">' + escapeHtml(doc.title) + "</a></h4>" +
      '<p class="muted-copy">' + escapeHtml(sourceMeta) + " · " + escapeHtml(doc.size_label || "") + "</p>" +
      "</div>" +
      ...
      "</div>" +
      "</div>" +
      '<p class="list-copy">' + escapeHtml(extra || doc.match_explanation || doc.content_preview || doc.diagnosis_summary) + "</p>" +
