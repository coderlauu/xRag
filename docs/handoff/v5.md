...
在 `Phase 2A` 可信问答闭环完成后，把 `Phase 2B` 从技术评估推进到正式 contract freeze，并进入 implementation freeze，为后续实现 lane 做准备。
...
- 在 implementation freeze 完成前直接进入 API、schema 或实现层 fan-out
- 开放互联网联网回答
- 浏览器插件
- 团队协作 / 权限模型
- 移动端
...
2. [Phase 2B Implementation Freeze Exec Plan](/Users/coderlauu/xRag/docs/exec-plans/active/2026-04-13-phase-2b-implementation-freeze.md)
...
- 当前最小事实源包括 `PRD / backlog / interaction delta / technical tradeoff / architecture / data-model / api / contract-freeze`
- 保持 `citation / refusal / freshness / release-readiness` 四条现有信任边界不被新需求破坏
- 在未完成 implementation freeze 前，不进入新的实现 lane 拆分

---

## 7. 进入 implementation freeze 前必须锁定的问题

1. `schema + shared-types + DTO/OpenAPI` 的主线程落地顺序如何固定？
2. `scope / citation / retrieval / history` 分别由哪些文件和模块持有所有权？
3. implementation lanes 切分时，哪些 surface 绝不允许子 agent 擅自改名或改语义？
4. `ops / P1-01` 继续如何保持 freeze-late，不反向拖慢 `P0` 主链？
5. 进入实现后最小验证集如何覆盖 `scope / history / evidence / retrieval` 四条主线？
...
- 当前 formal contract freeze 已完成，但 implementation freeze 仍未结束，不能误判为已经可以直接开并行实现 lane
