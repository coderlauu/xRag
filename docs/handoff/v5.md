...
在 `Phase 2A` 可信问答闭环完成后，把 `Phase 2B` 的 `Lane 0` contract-to-code 落到代码事实源，并进入 `Lane A / B` 的 feature implementation。
...
- `Lane 0` 已落地，后续 lane 只能在既有 contract 上实现，不重新打开 source-of-truth

---

## 7. 进入并行实现前必须锁定的问题

1. `Lane A / B` 是否继续保持不改 API path、字段命名、状态枚举和主数据模型语义？
2. `Lane C / D` 是否只消费 `Lane 0` 与 `Lane A / B` 已稳定的 shape，而不自行猜测后端结构？
3. 测试 lane 是否仍只在 feature lane 合流后启动？
...
- `Lane 0` 已落地，但 `apps/web/src/lib/api.ts`、`packages/shared-types/src/index.ts`、`apps/worker/src/database/answer-repository.ts` 仍是热点文件，后续 lane 需要严格遵守 ownership
