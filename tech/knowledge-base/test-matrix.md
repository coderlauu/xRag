...
**影响面**：一份百页 PDF 轻易就是几百个分块。也就是说**当前实现只能处理小文档**，这与 PRD 的验收标准直接冲突。

修复方向（未实施，待用户决定优先级）：批次之间加最小间隔 + 对 `429` 做指数退避重试，两者都属于 `OpenAiCompatibleEmbeddingClient` 内部，不影响调用方。需要注意重试要在**事务外**完成，这与现有的事务边界纪律一致。
