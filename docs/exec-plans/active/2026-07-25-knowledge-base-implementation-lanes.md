...
**完成定义**（状态截至 `2026-07-25`：**接口 14/19，两个后台机制完成、一个半完成**）
- [ ] 19 个接口全部实现，行为与 [API 契约](../../../tech/knowledge-base/api.md) 一致（含全部错误码）。**已实现 14 个**：知识库 5（工单 05）、文档上传/列表/详情/触发入库/任务历史 5（07、10）、分块查看/编辑/删除/新增/单条启禁用/批量启禁用 6 中的全部（13、14、15）——共 5+5+6=16 处端点，按契约计数的 19 个里**尚缺 5 个**：URL 来源上传（工单 16）、文档更新 / 删除 / 启禁用（工单 12）、以及定时同步涉及的配置更新（工单 17）。
- [x] `cd backend && ./mvnw -B verify` 通过（**103 条测试**，其中 2 条真实 API 用例在无 Key 时自动跳过）。
- [ ] 全部 `repository` 查询方法都带 `deleted = false`（这条靠约定维持、极易漏，需要逐个方法核对一遍）。**留给工单 18 专项核对**。已知**有意的例外两处**，都写进了方法注释：`ingestion_run` 的查询（日志性质记录，没有删除操作）、`DocumentChunkRepository.maxChunkIndex`（已删除分块的序号必须继续被占着，否则新分块会撞号）。
...
**完成定义**（状态截至 `2026-07-25`：**矩阵已完成，用例落地约七成，手工项 2/9**）
- [x] `test-matrix.md` 覆盖 [PRD §8](../../prd/2026-07-25-knowledge-base-prd.md) 全部 7 条验收标准，每条能追溯到具体用例（工单 02，8 个功能域 108 条用例 + 手工验收项 + 追溯表）。
- [x] `cd backend && ./mvnw -B verify` 通过，含新增的单元测试与集成测试（**103 条**）。
- [ ] 全部手工验证项已执行并把**实际结果**记录进矩阵。**已执行 2 项**：M-01（30MB→实测加到 48MB 的上传堆内存，通过，最高优先级风险已关闭）、M-06（真实 Ark API 连通性）。**M-07 / M-08 在工单 06 实测过但结果尚未回写矩阵**；M-02 / M-04 / M-05 / M-09 待对应工单；**CHK-36 已由手工改为自动化**，理由见矩阵 §6。

尚未落地的用例集中在三处，都跟着对应工单走：文档管理（DOC-2x，工单 12）、URL 与定时同步（SYNC-\*，工单 16/17）、逻辑删除专项（DEL-\*，工单 18）。
...
**完成定义**（状态截至 `2026-07-25`：**仅第一项达成，本 Lane 主体是工单 19，尚未开始**）
- [x] `scripts/ci-validate.sh` 本地全绿。
- [ ] CI 在 GitHub 上实际跑绿一次（包含新增的集成测试）。工单 07 时**越界预支了一步**：给 CI `validate` job 加了 Postgres 服务（不加的话那批集成测试推上去必红）。那是最小版本，工单 19 应把它当起点而非终点——RustFS、pgvector 扩展、假 Embedding 注入都还没在 CI 里验证过。
- [ ] 两份 env example 包含本模块全部新增配置项，无真实密钥。**配置项清单已定型**（Embedding 五项 + 上传大小与并发 + 定时同步两项 + `app.knowledge.ingestion.*`），可以开始登记。
- [ ] `deploy/README.md` 说明了 `EMBEDDING_API_KEY` 的管理方式，以及 CI 用假 Embedding 实现这一事实。**这条必须写**——`RealEmbeddingApiTests` 用 `@EnabledIfEnvironmentVariable` 守门、无 Key 自动跳过，不写清楚的话后来者会以为 CI 覆盖了真实 API 链路。
