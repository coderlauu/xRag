...
| KB-12 | 空知识库、无活动任务 | `DELETE` | `204`；知识库 `deleted=true` | 集成 |
| KB-13 | 同上，删除后 | `GET` 该 kb 的文档列表 | `404`（父资源已删，不是返回空列表——空列表会让调用方以为知识库还在） | 集成 |
| KB-14 | 知识库下有文档 | `DELETE` | `409 KB_NOT_EMPTY`，知识库与文档都未删除 | 集成 |
| KB-15 | 知识库有活动入库任务（同时也可有文档） | `DELETE` | 优先返回 `409 KB_HAS_ACTIVE_RUNS`，所有状态不变 | 集成 |
...
| SYNC-12 | 当前版本已成功；源站发布新内容，但新版本入库失败 | 触发扫描并等待 | 当前 `file_key`、分块和向量仍全部指向上一成功版本；失败任务保留自己的 `input_file_key` | 集成 |

## 6.1 STO — 对象与向量存储隔离

| 编号 | 前置状态 | 操作 | 预期结果 | 层级 |
|---|---|---|---|---|
| STO-01 | 上传一个文件 | 查看 RustFS Key | Key 按知识库 / 文档 / 版本分层，包含稳定别名与安全文件名 | 集成 |
| STO-02 | 一个被数据库引用的对象和一个孤儿对象 | dry-run 审计 | 只报告孤儿，不删除任何对象 | 集成 |
| STO-03 | 同上 | 显式永久清理 | 只删除超过宽限期的孤儿，引用对象保留 | 集成 |
| STO-04 | 文档刚被逻辑删除 | 在恢复宽限期内审计 | 原文件仍算引用，不进入孤儿清单 | 集成 |
| STO-05 | 文档逻辑删除已超过宽限期 | 审计 | 原文件进入孤儿清单；默认仍不自动删除 | 集成 |
| STO-06 | 同一 Bucket 有其他模块的对象 | 审计或永久清理 | 非 `knowledge-base(s)/` 前缀完全不进入扫描和删除范围 | 集成 |
| STO-07 | 两个知识库各有一个相似向量 | 按第一个 `kbId` 检索 | 结果只包含第一个知识库的分块 | 集成 |
