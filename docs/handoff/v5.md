...
在 `Phase 2A` 可信问答闭环完成后，把 `Phase 2B` 的 `Lane 0 / A / B` 落到代码事实源，并继续推进 `Lane C / D` 的 web 接线。
...
- `Lane 0 / A / B` 已落地；`api` 已完成 `recent history / continue lineage / evidence_groups` 的 read model，`worker` 已完成 `typed scope filters / answer_claims / claim-level citations / low_support exclusion`，后续 lane 只能在既有 contract 上实现，不重新打开 source-of-truth

---

## 7. 进入并行实现前必须锁定的问题

1. `Lane C / D` 是否只消费 `Lane 0 / A / B` 已稳定的 shape，而不自行猜测后端结构？
2. 测试 lane 是否仍只在 `Lane C / D` 合流后继续扩充？
3. `ops / P1-01` 是否继续保持 freeze-late，不反向拖慢 `P0` 主链？
4. 并行实现一旦出现 contract 漂移，是否立即切回主线程收口？
...
- `Lane 0 / A / B` 已落地，但 `apps/web/src/lib/api.ts`、`packages/shared-types/src/index.ts`、`apps/worker/src/database/answer-repository.ts` 仍是热点文件，后续 lane 需要严格遵守 ownership
