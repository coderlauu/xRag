...
**完成定义**（状态截至 `2026-07-25`：**三页已建成，文档管理操作待工单 12**）
- [x] `cd frontend && pnpm build` 与 `pnpm lint` 通过。
- [x] 在浏览器中实际完成一遍 §2 描述的可演示流程，不借助任何 API 调试工具（工单 13 闭环：创建知识库 → 上传 → 触发分块 → 看到分块列表 → 编辑分块 → 两表一致）。
- [ ] 上传超限文件、上传不支持格式、对处理中文档点删除，三种情况都能看到明确的中文错误提示。**前两种已验**（工单 07，415 实测）；第三种要等工单 12 做出删除文档按钮。
- [x] 轮询在页面切走后停止（工单 10 用应用内路由跳转验证，不是整页刷新——那会重置 performance 时间线什么都证明不了；切走后只多 1 次在途请求，实测间隔精确 2000ms）。

已建成页面：知识库列表、文档列表（含上传与处理记录抽屉）、分块管理（查看/筛选/分页/编辑/删除/新增/单条与批量启禁用）。**尚缺**：文档的编辑 / 启禁用 / 删除操作（工单 12）、URL 来源表单与定时同步配置（工单 16、17）。
