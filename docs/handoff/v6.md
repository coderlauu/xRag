...
在 `Phase 2B` 已完成的可信问答与可控问答基线上，把 `v6 / Phase 2C` 收敛为“质量治理与运行治理”版本：implementation freeze 已完成，当前进入 `implementation-lanes`，下一步由主线程先完成 `Lane 0: Contract To Code`。
...
- 主线程先完成 `Lane 0` 的 `schema / migration / shared-types / DTO / OpenAPI / SDK / web api adapter`
- 在 `Lane 0` 完成后，按计划切分 API read model、ingestion、web ops board 和 test lanes
- 更新 `current.md / status / exec plan`，保证后续新 session 可恢复

### 明确不做

- 绕过 `Lane 0` 直接进入并行 implementation lanes
...
- 按 [Phase 2C Implementation Lanes](/Users/coderlauu/xRag/docs/exec-plans/active/2026-04-16-phase-2c-implementation-lanes.md) 先完成 `Lane 0`
- `Lane 0` 完成前避免提前启动并行 lane 或超出版本边界的自动 remediation
...
1. `Lane 0` 如何把 `evaluation_runs / deployment_records / overview / trends` contract 落成代码事实源？
2. `Lane 0` 完成后，哪些 lane 可以并行，哪些必须继续主线程收口？
