...
- 本文描述的是 `Phase 2A` 的目标交互全景，不等于首批交付清单
- `P0` 首批交付必须先完成“可信问答闭环”：问答入口、混合检索、最小范围控制、证据链、拒答、freshness 可见性和评估门槛
- `P1` 再补扩展范围控制、检索实验台增强、完整运维面板和问题历史
...
`search.html` 作为 `Phase 2A` 的完整目标页保留在交互说明中，但其中更细的 retrieval trace 与 explain 能力优先级为 `P1`。
...
`ops.html` 的完整面板能力属于 `P1` 补强；`P0` 只要求评估门槛和最小上线守护已经定义清楚，不要求首批就把所有趋势和运维形态都产品化。
