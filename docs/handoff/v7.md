...
在 `Phase 2C` 已把质量治理与运行治理做成聚合主板后，把 `v7 / Phase 3A` 收敛为“深钻诊断与样本回放”版本：`implementation-freeze` 已完成，当前进入 `implementation-lanes`，下一步由主线程推进 `Lane 0: Contract To Code`。
...
- 在 `Lane 0` 完成前并行下放 `Phase 3A` 代码开发
...
3. [Phase 3A Implementation Freeze Exec Plan](/Users/coderlauu/xRag/docs/exec-plans/completed/2026-04-17-phase-3a-implementation-freeze.md)
...
9. [Phase 3A Product Backlog](/Users/coderlauu/xRag/docs/prd/2026-04-17-xrag-phase-3a-backlog.md)
...
2. `Lane A / B` 如何在不改 contract 的前提下分别实现 sample/compare 与 replay 聚合？
3. `Lane C / D` 如何覆盖 ops diagnostic UI、integration、e2e、smoke 和空数据场景？
