...
| M-09 | 三个页面的浏览器实测 | ⬜ **待项目所有者执行**——判据是"用户能不能理解界面在说什么"，只有人能判断 |
| M-10 | 处理进行中终止进程（工单 11 遗留） | ✅ 通过 `2026-07-28` |
| DOC-09 | 对象存储不可达时上传 | ✅ 通过 `2026-07-28` |

**九条自动可判的手工项里，两条抓到了真问题**（M-05 的 `max-request-size`、M-06 顺带发现的出站限流），都已修复并回归。这个比例值得记下来：如果它们继续以"有空再做"的状态挂着，两个缺陷都会活到生产。
