...
- `2026-07-25`: **Phase 1 Step D 阶段状态**：19 张工单完成 12 张（01~07、09、10、13~15），后端 103 条测试全绿，`scripts/ci-validate.sh` exit 0。逐工单的做法与偏离原因记在[任务书 Decision Log](2026-07-25-knowledge-base-implementation-lanes.md)，各 Lane 的完成度记在该文件 §4 每个 Lane 的完成定义块。概括：
  - **已打通并可在浏览器演示**：创建知识库 → 上传文件 → 触发分块 → 提取/切分/真实向量化/写库 → 界面查看、筛选、分页浏览分块 → 编辑、新增、单条与批量启用禁用 → 数据库与向量表始终一致。这正好是任务书 §2 定义的可演示状态。
  - **剩余 7 张**：08 上传限流、11 心跳超时回收（启动回收已在 10 顺带完成）、12 文档管理、16 URL 来源 → 17 定时同步、18 逻辑删除专项测试、19 运维/CI。Step E（矩阵回写）与 Step F（CI/CD）尚未到期。
  - **两条跨模块的经验，值得带进 04~06 模块**：
    1. **界面规格的评审重点应该是"状态组合的完整性"，不是逐条读文案。** 工单 13 与 14 各暴露一处 ui-spec 缺口（禁用分块保存后说"向量已更新"是错的；「可指定插入位置」有契约无控件），两处的共同点是**都不表现为文档冲突**——各份文档单独看都自洽，只有在写实现、把状态组合逐个走一遍时才暴露。做法应该是把维度列出来做叉乘，看每一格有没有说法。
    2. **能变成断言的手工验证项就该变。** CHK-36「Embedding 必须在事务外」原本要跑 psql 盯长事务，改成在假 Embedding 里记录调用时刻是否处在事务中之后，一条用例守住三条路径，而且验证过加上 `@Transactional` 它立刻变红。手工项的成本不只是执行一次，是**每次改动都得重新执行一次，于是实际上只执行了一次**。
  - **仍未解除的外部依赖**：`EMBEDDING_API_KEY` 只在提供 Key 的那次会话里注入过。凡是要写向量的界面操作（新增分块、启用分块、批量启用、编辑已启用分块）在无 Key 时只能验到 502 分支，正确路径靠集成测试的确定性假实现覆盖。Step E 的 M-09 需要一次带 Key 的完整浏览器走查。
