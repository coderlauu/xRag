...
在 `Phase 2A` 可信问答闭环完成后，把 `Phase 2B` 的 `Lane 0 / A / B / C / D` 落到代码事实源，并进入 `Lane E / F` 的测试与发布准备。
...
- `Lane 0 / A / B / C / D` 已落地；`web` 已完成 `typed scope filters / recent answers / continue asking / evidence groups / retrieval summary / search/detail jumpback`，下一步只进入 `Lane E / F`，补测试与 release-readiness，不重新打开 source-of-truth

---

## 7. 进入并行实现前必须锁定的问题

1. `Lane E` 是否只在既有 answers contract 上补 integration，不重新发明 fixture 或 shape？
2. `Lane F` 是否覆盖 `/search -> /ask -> /detail` 的关键跳转链，而不把细节绑定到脆弱 UI 文案？
3. `ops / P1-01` 是否继续保持 freeze-late，不反向拖慢 `P0` 主链？
4. 测试或 smoke 一旦暴露 contract 漂移，是否立即切回主线程收口？
